...
/**
 * The Line class represents a line in two-dimensional space. It provides methods for calculating the middle point
 * of the line and checking if two lines are equal. The class also includes constants for threshold values used in
 * double comparisons. The line is defined by two Point objects representing the start and end points of the line.
 */
public class Line {
    /**
    ...
    public Point intersectionWith(Line other) {
        ...
        if ((x < Math.min(x1, x2) && !doubleEquals(x, Math.min(x1, x2)))
                || (x > Math.max(x1, x2) && !doubleEquals(x, Math.max(x1, x2)))
                || (x < Math.min(x3, x4) && !doubleEquals(x, Math.min(x3, x4)))
                || (x > Math.max(x3, x4) && !doubleEquals(x, Math.max(x3, x4)))) {
            return null; // intersection point is outside the line segments
        }

        if ((y < Math.min(y1, y2) && !doubleEquals(y, Math.min(y1, y2)))
                || (y > Math.max(y1, y2) && !doubleEquals(y, Math.max(y1, y2)))
                || (y < Math.min(y3, y4) && !doubleEquals(y, Math.min(y3, y4)))
                || (y > Math.max(y3, y4) && !doubleEquals(y, Math.max(y3, y4)))) {
            return null; // intersection point is outside the line segments
        }
    ...
    public boolean isPointOnLine(Point p, Line line) {
        ...
        double crossProduct = (pDyiff * (line.end.getX() - line.start.getX()))
                - (pDxiff * (line.end.getY() - line.start.getY()));
    ...
    public boolean isOverlapping(Line a, Line b) {
        ...
        /* Check if the intersection point is within the bounds of both lines.
         If the lines intersect at the intersection point. Check if there are more than one intersection points,
         by checking if there is another intersection point within the bounds of both lines*/
    ...
    public Point closestIntersectionToStartOfLine(Rectangle rect) {
        ...
        for (Point intersectionPoint : intersectionPoints) {
            if (intersectionPoint != null) {
                double distance = intersectionPoint.distance(this.getStart());
